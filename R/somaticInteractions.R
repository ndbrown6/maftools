...
somaticInteractions = function(maf,
			       plot = FALSE,
			       top = 25,
			       genes = NULL,
			       pvalue = c(0.05, 0.01),
			       ...
			       limitColorBreaks = TRUE)
{
  ...
  interactions = do.call(rbind, mclapply(1:ncol(mutMat), function(i) sapply(1:ncol(mutMat), function(j) {f<- try(fisher.test(mutMat[,i], mutMat[,j]), silent=TRUE); if(class(f)=="try-error") NA else ifelse(f$estimate>1, -log10(f$p.val),log10(f$p.val))}), mc.cores = 12))
  oddsRatio = oddsGenes = do.call(rbind, mclapply(1:ncol(mutMat), function(i) sapply(1:ncol(mutMat), function(j) {f<- try(fisher.test(mutMat[,i], mutMat[,j]), silent=TRUE); if(class(f)=="try-error") f=NA else f$estimate}), mc.cores = 12))
  ...
  sigPairsTbl = data.table::rbindlist(
                          mclapply(X = seq_along(1:nrow(sigPairs)), function(i) {
                                   x = sigPairs[i,]
